...
def get_agent_pipeline() -> Pipeline:
    ...
    pp = Pipeline(
        metadata={
            "inputs": {
                "query": ["builder.query", "ci_parser.ci_output"],
                "repo": "ci_parser.repo",
                "issue_url": "ci_parser.issue_url", 
            },
    ...
    pp.add_component("ci_parser", ci_parser)
    pp.add_component("adapter", adapter)
    pp.add_component("builder", builder)

    # Function to convert CI output to ChatMessage
    def create_chat_message(ci_output: str) -> list[ChatMessage]:
        message_content = f"""# CI Failures Fix Request

The CI pipeline has identified failures that need to be fixed. 
...
1. Review the CI failures above carefully
2. Fix all failing tests, linting issues, and other errors
3. Create a pull request with your changes when complete"""
        return [ChatMessage.from_user(message_content)]

    # Connect components
    pp.connect("ci_parser.ci_output", (create_chat_message, "ci_output"))
    pp.connect(create_chat_message, "agent.messages")
    pp.connect("ci_parser.repo", "agent.repo")
    pp.connect("ci_parser.issue_url", "agent.issue_url")
