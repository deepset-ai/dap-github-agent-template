...
def get_agent_pipeline() -> Pipeline:
    ...
    # Components to process CI output
    ci_parser = CIOutputParser()
    branch_creator = CIBranchCreator(fail_if_exists=False)
    message_converter = CIDocumentToChatMessageConverter()
    ...
    pp = Pipeline(
        metadata={
            "inputs": {
                ...
                "repo": ["ci_parser.repo", "branch_creator.repo"],
            },
    ...
    pp.add_component("branch_creator", branch_creator)
    pp.add_component("message_converter", message_converter)
    pp.add_component("adapter", adapter)
    pp.add_component("builder", builder)

    pp.connect("ci_parser.documents", "message_converter.documents")
    pp.connect("ci_parser.repo", "branch_creator.repo")
    pp.connect("branch_creator.branch_name", "agent.branch")
    pp.connect("ci_parser.repo", "agent.repo")
    pp.connect("message_converter.messages", "agent.messages")
