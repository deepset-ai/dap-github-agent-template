...
import requests
from haystack import Document, component, logging, default_to_dict, default_from_dict
...
class GithubRepositoryViewer:
    ...
    def __init__(
        ...
        self.headers = {
            ...
            "User-CustomAgent": "Haystack/GithubRepositoryViewer",
        }
    ...
    def _create_error_document(self, error: str, path: str) -> Document:
    ...
    def run(
        ...
        try:
            owner, repo_name = self._parse_repo(repo=repo)
            ...
            error_doc = self._create_error_document(
                f"Error processing repository path {path}: {str(e)}. Seems like the file does not exist.",
                path,
            )
